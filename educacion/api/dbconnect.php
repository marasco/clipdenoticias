...
class db {
    ...
    function __construct() {
             $this->DB_HOST = '192.168.0.192';
            $this->DB_USERNAME = 'mysql_root'; // !!! CHANGE ME
            $this->DB_PASSWORD = 'fran21'; // !!! CHANGE ME
            $this->DB_DATABASE = 'clipping'; // !!! CHANGE ME
    ...
    function query($sql, $only_first = false) {
        ...
        if ($this->found_rows) {
            $rt = mysqli_fetch_row(self::$instance->query("SELECT FOUND_ROWS();"));
            $this->cant = $rt[0]; 
             
        }
        $this->nr = @mysqli_num_rows($result);
